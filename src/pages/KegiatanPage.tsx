...
export default function KegiatanPage() {
  ...
  const handleOpenEdit = (kegiatan: Kegiatan) => {
    ...
    setSelectedKegiatan({ ...kegiatan, tanggal: formattedDate });
    setIsModalOpen(true);
  };
  ...
  const handleFormSubmit = async (data: any) => {
    const payload = {
      ...data,
      tanggal: data.tanggal ? data.tanggal.substring(0, 10) : data.tanggal,
    };

    let success = false;
    if (selectedKegiatan && selectedKegiatan.id) {
      success = await updateKegiatan(selectedKegiatan.id, payload);
    } else {
      success = await addKegiatan(payload);
    }
    ...
    },
    {
      ...
      accessor: (row) => (
        <div className="flex items-center justify-end gap-2">
          ...
          <button
            onClick={() => row.id && removeKegiatan(row.id)}
